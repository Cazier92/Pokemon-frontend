...
const BattleScreen = (props: BattleScreenProps): JSX.Element => {
  ...
  useEffect((): void => {
    if (newPokemon) setStartTxt(`A wild ${newPokemon.name} appeared!`);
  }, [newPokemon]);
    ...
    if (partyPokemon)
      if (partyPokemon.moveSet[0].name) {
        ...
        const findMoves = async (): Promise<void> => {
          const updatedPokemon = await pokemonService.findPokemon(
            partyPokemon._id
          );
  ...
  useEffect((): void => {
    if (levelUpMoves.length && partyPokemon) {
      ...
      if (partyPokemon.moveSet.length >= 4) {
        setMustForget(true);
        setLearnMoveTxt(
          `${capPokemon(partyPokemon)} wants to learn ${
            levelUpMoves[0].name
          }, should a move be forgotten to learn ${levelUpMoves[0].name}?`
        );
      } else {
        setLearnMoveTxt(
          `${capPokemon(partyPokemon)} learned ${levelUpMoves[0].name}!`
        );
      }
  ...
  if (newPokemon && partyPokemon) {
    ...
    const expGain = async (): Promise<void> => {
      ...
      setOpponentFainted(false);
      setShowExpGain(true);
      const updatedPokemon = await pokemonService.expGain(
        partyPokemon._id,
        newPokemon._id
      );
      setPartyPokemon(updatedPokemon);
    ...
    const handleLearnNextMove = async (): Promise<void> => {
      ...
      const updatedPokemon = await pokemonService.newMove(
        partyPokemon._id,
        newMove
      );
      setPartyPokemon(updatedPokemon);
    ...
    const evolve = async (): Promise<void> => {
      ...
      if (partyPokemon.evolves && partyPokemon.evolvesTo[0].minLevel) {
        if (partyPokemon.level >= partyPokemon.evolvesTo[0].minLevel) {
          ...
          if (typeof updatedPokemon !== "string") {
            setEvolveTxt(
              `${capPokemon(preEvolve)} evolved into ${capPokemon(
                updatedPokemon
              )}!`
            );
            setShowEvolve(true);
    ...
    const handleUseBall = async (): Promise<void> => {
      if (ball) {
        ...
        const ballResponse = await battleService.useBall(
          ball._id,
          newPokemon._id
        );
        setBall(undefined);
        ...
        if (typeof ballResponse[1] === "string") {
          if (ballResponse[2] === true) {
            const foundOpponent = await pokemonService.findPokemon(
              newPokemon._id
            );
            ...
            const randomNum = Math.floor(
              Math.random() * opponentMoveSet.length
            );
            const opponentMoveId = opponentMoveSet[randomNum];
            const opponentMove = await battleService.findMove(
              opponentMoveId._id
            );
        ...
        if (med) {
          if (med.ether) {
            ...
            if (etherMove) {
              ...
              const medResponse = await battleService.useMedicine(
                med._id,
                pokemon._id,
                etherMove
              );
    ...
    const handleChooseMove = async (
      ...
      if (move.currentPP > 0) {
        ...
        if (moves[0].priority > moves[1].priority) {
          ...
          const updatedOpponent = await battleService.useMove(
            moves[0]._id,
            userPok._id,
            foundOpponent._id
          );
          setNewPokemon(updatedOpponent);
          setOpponentHealthPer(
            75 * (updatedOpponent.currentHP / updatedOpponent.totalHP)
          );
          const updatedUser = await pokemonService.findPokemon(
            partyPokemon._id
          );
          ...
          setShowBattleText(true);
          const updatedUser = await battleService.useMove(
            moves[1]._id,
            foundOpponent._id,
            userPok._id
          );
          setPartyPokemon(updatedUser);
          setPlayerHealthPer(
            75 * (updatedUser.currentHP / updatedUser.totalHP)
          );
          const updatedOpponent = await pokemonService.findPokemon(
            newPokemon._id
          );
          setNewPokemon(updatedOpponent);
    ...
    const handleNextMove = async (
      ...
      if (attacker.currentHP > 0) {
        ...
        if (opponentTurn) {
          ...
          setUserProfile(
            profilesData.find((profile) => profile._id === user?.profile)
          );
        } else {
          const updatedOpponent = await battleService.useMove(
            move._id,
    ...
    const partyPokemonExpPercent =
      (partyPokemonCurrentExp - partyPokemonLevelBase) /
      (partyPokemonNextLevel - partyPokemonLevelBase);
      ...
      return (
        <div className="battle-screen">
                ...
                <div className="health-bar">
                  ...
                  <div className="health-sub">
                    <div
                      ...
                      id="opponent-health-empty"
                    ></div>
                    <div
                      className="health-percent"
                      id="opponent-health-percent"
                    ></div>
                  </div>
                </div>
              </div>
          ...
          {med && showEtherTxt && etherMove && etherPok ? (
            <div
              className="opponent-faint"
              onClick={() => handleUseMedicine(etherPok)}
            >
              <p className="opponent-faint-txt">
                You used {med.name} on {capPokemon(etherPok)}!
              </p>
            </div>
          ) : (
            <></>
          ...
          {showExpGain ? (
            <div className="opponent-faint" onClick={() => levelUp()}>
              <p className="opponent-faint-txt">
                {capPokemon(partyPokemon)} gained experience for defeating the
                wild {capPokemon(newPokemon)!}
              </p>
            </div>
          ) : (
            <></>
          ...
          {showLevelUp ? (
            <div className="opponent-faint" onClick={() => checkForMoves()}>
              <p className="opponent-faint-txt">
                {capPokemon(partyPokemon)} leveled up to level{" "}
                {partyPokemon.level}!
              </p>
            </div>
          ) : (
          ...
          {showLevelUpMoves ? (
            <div className="opponent-faint">
              ...
              {mustForget ? (
                <>
                  <button onClick={() => handleChooseForget()}>
                    Forget Move
                  </button>
                  <button onClick={() => handleChooseNotLearn()}>
                    Don't Learn
                  </button>
                </>
              ) : (
          ...
          {showMedMsg && medMsg && secondMove && attacker && target ? (
            <div
              className="opponent-faint"
              onClick={() =>
                handleNextMove(secondMove, attacker, target, opponentTurn)
              }
            >
              ...
              <div className="health-box" id="user-health">
                ...
                <div className="health-bar">
                  ...
                  <div className="health-sub">
                    <div
                      className="health-empty"
                      id="player-health-empty"
                    ></div>
                    <div
                      className="health-percent"
                      id="player-health-percent"
                    ></div>
                  </div>
                </div>
              </div>
                  ...
                  {showMoves ? (
                    partyPokemon.moveSet.map((move) => (
                      <div
                        ...
                        key={move._id}
                        onClick={() =>
                          handleChooseMove(move, partyPokemon, newPokemon)
                        }
                      >
                  ...
                  <>
                    <div className="ether-move-menu">
                      <div className="ether-txt-div">
                        <p className="ether-menu-txt">
                          Please select one of {capPokemon(etherPok)}'s moves to
                          restore PP:
                        </p>
                      ...
                      <div className="ether-move-selection">
                        {etherPok.moveSet.map((move) => (
                          <div
                            className="move"
                            key={move._id}
                            onClick={() => handleChooseEtherMove(move)}
                          >
                            <p className="move-name">{move.name}</p>
                            <p className="move-pp">
                              {move.currentPP}/{move.totalPP}
                  ...
                  <div className="party-list">
                    {fullParty.map((pokemon) => (
                      ...
                      >
                        ...
                        <p className="party-hp">
                          {Math.floor(pokemon.currentHP)}/
                          {Math.floor(pokemon.totalHP)} HP
                        </p>
                ...
                <div className="party-menu">
                  {fullParty.map((pokemon) => (
                    ...
                    >
                      ...
                      <p className="party-level">Lv: {pokemon.level}</p>
                      <p className="party-hp">
                        {Math.floor(pokemon.currentHP)}/{pokemon.totalHP} HP
                      </p>
                      {pokemon._id === partyPokemon._id ||
                      pokemon.currentHP <= 0 ? (
                        <></>
                ...
                <div className="party-menu">
                  ...
                  <div className="med-party-list">
                    {fullParty.map((pokemon) => (
                      <div
                        className="pokemon"
                        onClick={() => handleUseMedicine(pokemon)}
                      >
                        <p>{capPokemon(pokemon)}</p>
                        <img src={pokemon.spriteFront} alt="" />
                        <p className="party-level">Lv: {pokemon.level}</p>
              ...
              {showPack ? (
                <div className="pack-menu">
                  <div className="pocket-selection">
                    <button onClick={() => handleShowMedicine()}>
                      Medicine
                    </button>
                  ...
                  {showMedicine && pack ? (
                    <div className="item-list">
                      {pack.medicinePocket.map((medicine) => (
                        <div
                          className="item"
                          onClick={() => handleChooseMedicine(medicine)}
                        >
                          <p>{medicine.name}</p>
